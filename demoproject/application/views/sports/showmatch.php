<!--  CONTENT  -->
<section id="content">
			<!-- row -->
				<div class="row">
					<div class="col-md-12">
						<section class="boxs ">
							<div class="boxs-header">
								<h3 class="custom-font hb-cyan">
									<strong>Sports</strong> Records</h3>

							</div>
							<div class="boxs-body">
								<div class="form-group">
									<label for="filter" style="padding-top: 5px">Search:</label>
									<input id="filter" type="text" class="form-control rounded w-md mb-10 inline-block" />
								</div>
								<table id="searchTextResults" data-filter="#filter" data-page-size="5" class="footable table table-custom">
									<thead>
										<tr>
                                            <th>match_id</th>
                                            <th>category_id</th>
                                            <th>subcategory_id</th>
                                            <th>match_title</th>
                                            <th>team1_name</th>
                                            <th>team2_name</th>
                                            <th>match_date</th>
                                            <th>stadium_id</th>
                                            <th>match_img</th>
                                            <th>match_banner</th>
                                            <th>match_details</th>
                                            <th>match_status</th>
                                            <th>match_dateregister</th>
											<!-- <th>Update</th>
											<th>Delete</th> -->
										</tr>
									</thead>
									<tbody>
                                              <?php
                                              $data=$this->db->get(' sportsmatches')->result();
                                                foreach($data as $row)
                                                {
                                                    ?>
                                                    <tr>
                                                        <td><?php echo $row->match_id ?></td>
                                                        <td><?php echo $row->category_id?></td>
                                                        <td><?php echo $row->subcategory_id ?></td>
														<td><?php echo $row->match_title ?></td>
                                                        <td><?php echo $row->team1_name ?></td>
                                                        <td><?php echo $row->team2_name ?></td>
                                                        <td><?php echo $row->match_date ?></td>
                                                        <td><?php echo $row->stadium_id ?></td>
                                                        <td><img src="<?php echo base_url() ?>/images/<?php echo $row->match_img ?>" alt="" height="100px"></td>
                                                        <td><img src="<?php echo base_url() ?>/images/<?php echo $row->match_banner ?>" alt="" height="100px"></td>
                                                        <td><?php echo $row->match_details ?></td>
														<?php 
															if($row->match_status == "unblocked")
															{
																?>
																	<td><a class="btn btn-raised btn-success btn-xs" href="<?php echo site_url() ?>/sportsmatch_controller/update/<?php echo $row->match_id ?>">
																	<?php echo $row->match_status ?></a></td>
																<?php
															}
															else
															{
																?>
																	<td><a class="btn btn-raised btn-danger btn-xs" href="<?php echo site_url() ?>/sportsmatch_controller/update/<?php echo $row->match_id ?>">
																	<?php echo $row->match_status ?></a></td>
																<?php
															}
														?>
                                                        <td><?php echo $row->match_dateregister ?></td>
														<!-- <td><a class="fa fa-edit (alias)" href="<?php echo site_url() ?>/sports_controller/edit_sports/<?php echo $row->sports_id ?>">UPDATE</a></td>
														<td><a class="fa  fa-trash-o" href="<?php echo site_url() ?>/sports_controller/delete_sports/<?php echo $row->sports_id ?>">DELETE</a></td> -->
													</tr>
                                                <?php 
                                                }
                                              ?>
                                    </tbody>
									<tfoot class="hide-if-no-paging">
										<tr>
											<td colspan="5" class="text-right">
												<ul class="pagination">
												</ul>
											</td>
										</tr>
									</tfoot>
								</table>
							</div>
						</section>
					</div>
				</div>
			</div>
		</section>
		<!--/ CONTENT -->

	</div>
	<!--/ Application Content -->

	<!--  Vendor JavaScripts  -->
	<script src="<?php echo base_url() ?>assets/bundles/libscripts.bundle.js"></script>
	<script src="<?php echo base_url() ?>assets/bundles/vendorscripts.bundle.js"></script>
	<script src="<?php echo base_url() ?>assets/js/vendor/footable/footable.all.min.js"></script>
	<!--/ vendor javascripts -->

	<!--  Custom JavaScripts  -->
	<script src="<?php echo base_url() ?>assets/bundles/mainscripts.bundle.js"></script>	<!-- Custom Js -->

	<!--  Page Specific Scripts  -->
	<script >
		$(window).load(function () {
			$('.footable').footable();
		});
	</script>
</body>

<!-- Mirrored from thememakker.com/templates/falcon/html/tables-footable.html by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 30 Dec 2019 06:44:54 GMT -->
</html>